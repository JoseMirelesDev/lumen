...
pub trait RendererSealed {
    ...
    /// Lumen fork addition: called when a property read during the last render
    /// changed (i.e. the scene graph is dirty in a way the renderer must
    /// re-trace). Renderers use this to skip the per-render item-dirty
    /// traversal when the ONLY reason to repaint is externally-marked regions
    /// (e.g. the animation slot's `mark_dirty_region`).
    fn property_changed(&self) {}

    /// Register a sequence of pre-rendered RGBA frames under `key`, for display by an
    /// `AnimationImage` element whose `animation-key` matches. All frames must share the
    /// same size. This is a Lumen fork addition (see docs/performance-fork-design.md):
    /// the content lives outside the reactive scene graph, so switching frames through
    /// [`Self::set_animation_frame`] never invalidates bindings or re-renders the tree.
    #[cfg(feature = "std")]
    fn register_animation_frames(
        &self,
        _key: u32,
        _frames: Vec<crate::graphics::SharedPixelBuffer<crate::graphics::Rgba8Pixel>>,
    ) -> Result<(), crate::api::PlatformError> {
        Err("This renderer does not support animation frames.".into())
    }

    /// Switch the frame shown by the `AnimationImage` with `animation-key == key`.
    /// Repaints only that item's rectangle (and schedules a redraw); the scene tree
    /// is not invalidated.
    fn set_animation_frame(&self, _key: u32, _frame_index: u32) {}
