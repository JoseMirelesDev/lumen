...
impl Window {
    ...
    /// Mark a region of the window as dirty, so that the renderer repaints only
    /// that region on the next frame (instead of the whole window), WITHOUT
    /// invalidating any property bindings in the scene graph.
    ///
    /// This is the escape hatch for external/animated content (e.g. a texture
    /// updated from a native thread or a custom render pass): after updating
    /// the content, call this with the affected rectangle and [`Self::request_redraw`],
    /// and only that region is repainted. The rest of the scene tree is not
    /// re-evaluated or re-traversed.
    ///
    /// This is a Lumen fork addition (see docs/performance-fork-design.md).
    pub fn mark_dirty_region(&self, region: crate::lengths::LogicalRect) {
        self.0.window_adapter().renderer().mark_dirty_region(region.into());
    }

    /// Register a sequence of pre-rendered RGBA frames under `key`, for display by an
    /// [`crate::items::AnimationImage`] element whose `animation-key` matches. All frames
    /// must share the same size.
    ///
    /// The frames live outside the reactive scene graph: switching the visible frame with
    /// [`Self::set_animation_frame`] only repaints the item's own rectangle (via
    /// [`Self::mark_dirty_region`]) and never invalidates property bindings or re-renders
    /// the rest of the scene.
    ///
    /// This is a Lumen fork addition (see docs/performance-fork-design.md).
    #[cfg(feature = "std")]
    pub fn register_animation_frames(
        &self,
        key: u32,
        frames: Vec<crate::graphics::SharedPixelBuffer<crate::graphics::Rgba8Pixel>>,
    ) -> Result<(), crate::platform::PlatformError> {
        self.0.window_adapter().renderer().register_animation_frames(key, frames)
    }

    /// Switch the frame shown by the [`crate::items::AnimationImage`] with
    /// `animation-key == key`. Repaints only that item's rectangle and schedules a
    /// redraw; the scene tree is not invalidated.
    ///
    /// This is a Lumen fork addition (see docs/performance-fork-design.md).
    pub fn set_animation_frame(&self, key: u32, frame_index: u32) {
        self.0.window_adapter().renderer().set_animation_frame(key, frame_index);
    }
