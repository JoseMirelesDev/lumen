...
pub trait ItemRenderer {
    ...
    /// Draw an [`AnimationImage`](crate::items::AnimationImage) item: content is
    /// provided out-of-band through `Window::register_animation_frames` /
    /// `set_animation_frame` and never invalidates the scene tree. Renderers that
    /// don't support it draw nothing.
    fn draw_animation_image(
        &mut self,
        _image: Pin<&crate::items::AnimationImage>,
        _self_rc: &ItemRc,
        _size: LogicalSize,
    ) {
    }
