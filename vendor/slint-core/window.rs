...
impl crate::properties::PropertyDirtyHandler for WindowRedrawTracker {
    fn notify(self: Pin<&Self>) {
        if let Some(window_adapter) = self.window_adapter_weak.upgrade() {
            // Lumen fork: let the renderer know a property changed so it can
            // decide whether the per-render item-dirty traversal is needed
            // (see `RendererSealed::property_changed`).
            window_adapter.renderer().property_changed();
