...
use crate::items::{
    AnimationImage, BoxShadow, Clip, ItemRc, ItemRef, Layer, Opacity, RenderingResult, TextInput,
};
use crate::lengths::{
...
impl<T: ItemRenderer + ItemRendererFeatures> ItemRenderer for PartialRenderer<'_, T> {
    ...
    forward_rendering_call!(fn draw_animation_image(AnimationImage));
...
pub struct PartialRenderingState {
    ...
    /// Lumen fork: when set, `apply_dirty_region` skips the per-item dirty
    /// traversal (`compute_dirty_regions`). The renderer sets this when no
    /// property changed since the last render and the only repaint driver is
    /// an externally-marked region (e.g. `Window::mark_dirty_region` from the
    /// animation slot), which saves a full tree walk per frame.
    pub skip_item_dirty_compute: Cell<bool>,
}
...
impl PartialRenderingState {
    ...
    pub fn apply_dirty_region<T: ItemRenderer + ItemRendererFeatures>(
        ...
        if !self.skip_item_dirty_compute.replace(false) {
    ...
    /// Lumen fork: re-register the current binding's dependencies (the window
    /// redraw tracker, during `draw_contents`) on every item's geometry and
    /// rendering tracker, WITHOUT computing dirty regions or mutating the
    /// cache.
    ///
    /// The per-item dirty traversal (`compute_dirty_regions`) is what keeps
    /// the redraw tracker subscribed to the whole scene: it reads each item's
    /// geometry (registering geometry properties, incl. Flickable viewport
    /// offsets, as dependencies) and registers the per-item rendering trackers
    /// as dependencies too. When a frame skips that traversal (no property
    /// changed since the last render — the only repaint driver is an
    /// externally-marked region like the animation slot), the tracker's
    /// dependency set is rebuilt from just the items that were actually drawn,
    /// and everything else is unsubscribed. A later scroll then changes
    /// `viewport-y` without notifying the tracker, its renders skip the
    /// traversal as well, and the moved content is never repainted (ghosting).
    /// Run this on skip frames to keep the tracker wired without paying for
    /// the full traversal.
    pub fn register_dependencies(&self, components: &[(ItemTreeWeak, LogicalPoint)]) {
        for (component, _origin) in components {
            let Some(component) = crate::item_tree::ItemTreeWeak::upgrade(component) else {
                continue;
            };
            crate::item_tree::visit_items(
                &component,
                crate::item_tree::TraversalOrder::BackToFront,
                |component, item, index, _state| {
                    let item_rc = ItemRc::new(component.clone(), index);
                    // Tracked read: registers the item's geometry properties
                    // (including Flickable viewport offsets) as dependencies
                    // of the current binding (the window redraw tracker).
                    let _ = item_rc.geometry();
                    // Same tracked read the full traversal performs via
                    // `CachedItemBoundingBoxAndTransform::new`: Transform /
                    // Rotate items expose their child transform properties
                    // (origin/scale/rotation) here, which would otherwise be
                    // pruned from the redraw tracker's dependencies on skip
                    // frames (a transform animation would stop notifying).
                    let _ = item_rc.children_transform();
                    // Re-subscribe the item's rendering tracker (its deps were
                    // recorded at its last draw), so property changes that
                    // dirty it also dirty the redraw tracker.
                    let rendering_data = item.cached_rendering_data_offset();
                    if let Some(entry) =
                        rendering_data.get_entry(&mut self.partial_cache.borrow_mut())
                    {
