...
impl FlickableDataInner {
    ...
    fn process_wheel_event(
        ...
        if std::env::var_os("LUMEN_DEBUG_RENDER").is_some() {
            crate::debug_log!(
                "[lumen-wheel] pos=({},{}) delta=({},{}) vp=({},{}) geom=({},{}) vw={} vh={} allowed={}",
                position.x,
                position.y,
                delta.x,
                delta.y,
                flick.viewport_x().get(),
                flick.viewport_y().get(),
                flick_rc.geometry().width(),
                flick_rc.geometry().height(),
                flick.viewport_width().get(),
                flick.viewport_height().get(),
