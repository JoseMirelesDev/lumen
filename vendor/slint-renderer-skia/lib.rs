...
/// Lumen fork: registry of pre-rendered animation frames, displayed by the
/// `AnimationImage` item without going through the reactive scene graph.
mod animation;
...
pub struct SkiaRenderer {
    ...
    /// Pre-rendered animation frames registered through `Window::register_animation_frames`.
    /// Lives here (not in the scene graph) so switching frames never invalidates bindings.
    animation_registry: animation::AnimationRegistry,
    /// Lumen fork: set by `property_changed` (a property read during the last
    /// render changed). When false at render time, the only repaint driver can
    /// be an externally-marked region (`mark_dirty_region` from the animation
    /// slot), so the per-item dirty traversal is skipped.

    rendering_metrics_collector: RefCell<Option<Rc<RenderingMetricsCollector>>>,
    properties_changed_since_render: Cell<bool>,
...
impl SkiaRenderer {
    ...
    pub fn default(context: &SkiaSharedContext) -> Self {
        Self {
            ...
            animation_registry: Default::default(),
            rendering_metrics_collector: Default::default(),
            properties_changed_since_render: Cell::new(true),
    ...
    pub fn set_surface(&self, surface: Box<dyn Surface + 'static>) {
        ...
        // Cached GPU surfaces reference the old graphics context; keep the
        // registered frames and rebuild the storage lazily at the next draw.
        self.animation_registry.reset_storage();
    ...
    fn render_components_to_canvas(
...

        {
            ...
            if let Some(partial_rendering_state) = self.partial_rendering_state() {
                ...
                // Lumen fork: if no property changed since the last render, the
                // only repaint driver can be an externally-marked region (the
                // animation slot's `mark_dirty_region`), so the per-item dirty
                // traversal is unnecessary — skip it and repaint just the
                // marked regions (plus the buffer-age history, applied below).
                //
                // BUT the traversal is also what keeps the window redraw
                // tracker subscribed to the whole scene; skipping it prunes
                // the tracker's dependencies down to the drawn items, so a
                // later scroll stops notifying it (ghosting). Re-register the
                // dependencies on skip frames with a cheap pass that does not
                // compute dirty regions or mutate the cache.
                let force_traversal =
                    std::env::var_os("LUMEN_TEST_NO_SKIP").is_some();
                let skip = !properties_changed && !force_traversal;
                partial_rendering_state.skip_item_dirty_compute.set(skip);
                if skip {
                    let deps_start = std::time::Instant::now();
                    partial_rendering_state.register_dependencies(components);
                    if std::env::var_os("LUMEN_DEBUG_RENDER").is_some() {
                        i_slint_core::debug_log!(
                            "[lumen-render] deps_pass {:?}",
                            deps_start.elapsed(),
                        );
                    }
                }
                ...
                if std::env::var_os("LUMEN_DEBUG_RENDER").is_some() {
                    let initial: usize =
                        partial_renderer.dirty_region.iter().count();
                    i_slint_core::debug_log!(
                        "[lumen-render] props_changed={properties_changed} skip={} initial_dirty={initial}",
                        !properties_changed,
                    );
                }
                ...
                if std::env::var_os("LUMEN_DEBUG_RENDER").is_some() {
                    let painted: usize = partial_renderer.dirty_region.iter().count();
                    let area_to_repaint: f32 =
                        partial_renderer.dirty_region.iter().map(|b| b.area()).sum();
                    i_slint_core::debug_log!(
                        "[lumen-render] frame_region={} painted={} ({:.2}%)",
                        dirty_region_for_this_frame.iter().count(),
                        painted,
...
impl i_slint_core::renderer::RendererSealed for SkiaRenderer {
    ...
    fn mark_dirty_region(&self, region: DirtyRegion) {
        if std::env::var_os("LUMEN_DEBUG_RENDER").is_some() {
            i_slint_core::debug_log!("[lumen-render] mark_dirty_region {} rect(s)", region.iter().count());
        }
        if let Some(partial_rendering_state) = self.partial_rendering_state() {
    ...
    fn property_changed(&self) {
        if std::env::var_os("LUMEN_DEBUG_RENDER").is_some() {
            i_slint_core::debug_log!("[lumen-render] property_changed");
        }
        self.properties_changed_since_render.set(true);
    }

    fn register_animation_frames(
        &self,
        key: u32,
        frames: Vec<i_slint_core::graphics::SharedPixelBuffer<i_slint_core::graphics::Rgba8Pixel>>,
    ) -> Result<(), PlatformError> {
        self.animation_registry.register(key, frames)
    }

    fn set_animation_frame(&self, key: u32, frame_index: u32) {
        let Some(rect) = self.animation_registry.set_frame(key, frame_index) else {
            if std::env::var_os("LUMEN_DEBUG_FIRE").is_some() {
                i_slint_core::debug_log!(
                    "[lumen-fire] set_frame({key}, {frame_index}): no rect (never drawn / identical)"
                );
            }
            return;
        };
        if std::env::var_os("LUMEN_DEBUG_FIRE").is_some() {
            i_slint_core::debug_log!(
                "[lumen-fire] set_frame({key}, {frame_index}): mark {:?}",
                rect,
            );
        }
        // Repaint only the item's rectangle; the rest of the scene tree is not
        // re-evaluated (the item's rendering tracker is clean).
        self.mark_dirty_region(rect.into());
        if let Ok(window_adapter) = self.window_adapter() {
