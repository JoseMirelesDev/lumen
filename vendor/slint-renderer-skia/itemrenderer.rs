...
use i_slint_core::items::{
    AnimationImage, ImageFit, ImageRendering, ItemRc, Layer, Opacity, RenderingResult,
};
...
pub struct SkiaItemRenderer<'a> {
    ...
    /// Pre-rendered animation frames (Lumen fork), keyed by `animation-key`.
    animation_registry: &'a crate::animation::AnimationRegistry,
}
...
impl ItemRenderer for SkiaItemRenderer<'_> {
    ...
    fn draw_animation_image(
        &mut self,
        image: Pin<&AnimationImage>,
        _self_rc: &ItemRc,
        size: LogicalSize,
    ) {
        let geometry = PhysicalRect::from(size * self.scale_factor);
        ...
        let key = image.animation_key() as u32;
        let Some(slot) = self.animation_registry.get(key) else { return };

        // Remember the item's on-screen rect so `Window::set_animation_frame` can
        // repaint exactly this region (logical coords, like the partial renderer).
        let local_to_device = self.canvas.local_to_device_as_3x3();
        let (device_rect, _) = local_to_device.map_rect(skia_safe::Rect::from_xywh(
            0.,
            0.,
            geometry.width(),
            geometry.height(),
        ));
        slot.record_rect(LogicalRect::new(
            LogicalPoint::new(
                device_rect.left() / self.scale_factor.get(),
                device_rect.top() / self.scale_factor.get(),
            ),
            LogicalSize::new(
                device_rect.width() / self.scale_factor.get(),
                device_rect.height() / self.scale_factor.get(),
            ),
        ));

        // GPU-backed: builds the atlas (all frames) once on first draw, then
        // returns the atlas image + the source-rect of the current frame. The
        // CPU buffers are released after the atlas is built (no RAM retention).
        let (skia_image, src) = match slot.update_surface(self.canvas) {
            Some(pair) => pair,
            None => {
                // GPU surface failed: CPU fallback (single frame raster).
                let Some(cpu_img) = slot.draw_cpu() else { return };
                let src = skia_safe::Rect::from_xywh(
                    0.,
                    0.,
                    cpu_img.width() as f32,
                    cpu_img.height() as f32,
                );
                (cpu_img, src)
            }
        };

        let dst = to_skia_rect(&geometry);
        let sampling: skia_safe::sampling_options::SamplingOptions =
