...
impl Default for Delay {
    fn default() -> Self {
        Self {
            ...
            num_filters: 5,
            // P2 tuning (see Cargo.toml sonora-aec3 fork + audio.rs SonoraAec doc):
            // delay_headroom 32→64 (+32 samples) and hysteresis 1→2 give
            // headroom for PipeWire jitter. Bisection proved it does NOT affect
            // agc_probe variance (ratio 5.52 stable across tunings), so kept for
            // AEC benefit (delay 4ms vs 8ms upstream). See task bisection.
            delay_headroom_samples: 64,
            hysteresis_limit_blocks: 2,
    ...
    fn default() -> Self {
        Self {
            refined: RefinedConfiguration {
                // P2 tuning: refined/coarse length_blocks 13→16 (≈64ms tail)
                // helps echo tail coverage. Bisection: no effect on agc_probe
                // variance, kept for AEC suppression -110dB.
                length_blocks: 16,
            ...
            // P2 tuning: initial_state 2.5→1.0 accelerates convergence (delay
            // 4ms vs 8ms). Bisection showed no agc_probe variance change, so
            // kept — tradeoff documented in audio.rs SonoraAec.
...
impl Default for DominantNearendDetection {
    fn default() -> Self {
        Self {
            // P2 tuning: DTD enr 0.25→0.35, snr 30→20, hold 50→70 — more sensitive
            // nearend detection (less suppression of nearend). Bisection proved
            // no agc_probe variance impact, kept for voice preservation.
            enr_threshold: 0.35,
...
impl Default for HighBandsSuppression {
    fn default() -> Self {
        Self {
            ...
            // P2 tuning: anti_howling 400/1.0→200/0.30 — anti-howling.
            // Bisection: no agc_probe effect, kept for Larsen protection.
            anti_howling_activation_threshold: 200.0,
            anti_howling_gain: 0.30,
        }
    }
}
        ...
        Self {
            ...
            normal_tuning: Tuning {
                mask_lf: MaskingThresholds {
                    // P2 tuning: mask_lf enr_suppress 0.4→0.30 — more aggressive
                    // low-freq suppression. Bisection: no agc_probe variance
                    // impact, kept for echo suppression tuning.
                    enr_transparent: 0.3,
                    enr_suppress: 0.30,
                    emr_transparent: 0.3,
                },
                mask_hf: MaskingThresholds {
                    ...
                    // P2 tuning: mask_hf enr_suppress 0.1→0.08
                    enr_suppress: 0.08,
                    emr_transparent: 0.3,
                },
                max_inc_factor: 2.0,
            ...
            // P2 tuning: use_subband_nearend_detection false→true and
            // conservative_hf_suppression false→true — improve nearend
            // detection granularity. Bisection proved neither affects
            // agc_probe ratio (5.52 stable) so kept for AEC benefit.
