...
static void fe_fft_dispatch_select(void) {
    ...
    /* x86 FFT: SSE2 is the fallback, AVX2 is the fast path.
     * fe_qgemm_init's minimum is now SSE4.1, so SSE2 is always available
     * on x86_64 (baseline). AVX2 overwrites if present; AVX-512 overwrites
     * AVX2 when available. */
    uint32_t caps = fe_cpu_x86_caps();
    if (caps & FE_X86_HAS_AVX2) {
        g_fft_fwd = fe_fft_forward_avx2;
        g_fft_inv = fe_fft_inverse_avx2;
    } else {
        g_fft_fwd = fe_fft_forward_sse2;
        g_fft_inv = fe_fft_inverse_sse2;
    }
...
#else
#  error "unsupported ISA — faster-enhancer.c requires aarch64 (NEON) or x86_64 (SSE2)"
#endif
}
